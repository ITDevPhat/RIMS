...
async function authenticate(page: Page, options: { emptyAlerts?: boolean } = {}) {
  await page.route("http://localhost:5000/api/**", async (route) => {
    ...
    if (route.request().url().includes("/dashboard/deadlines")) {
      const upcoming = {
        deadlineId: 101, deadlineType: "Nộp báo cáo tiến độ định kỳ", title: "Báo cáo tiến độ",
        dueDate: "2026-09-01", priorityLevel: "high", deadlineStatus: "open",
        projectId: 1, projectCode: ganttProjects[0].projectCode, projectTitle: ganttProjects[0].projectTitle,
        responsibleUserName: ganttProjects[0].principalInvestigatorName, daysRemaining: 25, isOverdue: false,
      };
      const overdue = { ...upcoming, deadlineId: 102, dueDate: "2026-01-01", daysRemaining: -203, isOverdue: true, priorityLevel: "critical" };
      await route.fulfill({ json: { success: true, data: options.emptyAlerts
        ? { upcomingIn7Days: [], upcomingIn30Days: [], overdue: [], ethicsExpiring: [], trainingEventsUpcoming: [] }
        : {
          upcomingIn7Days: [], upcomingIn30Days: [upcoming], overdue: [overdue],
          ethicsExpiring: [{ projectId: 1, projectCode: ganttProjects[0].projectCode, projectTitle: ganttProjects[0].projectTitle, ethicsExpiryDate: "2026-11-20", principalInvestigatorName: ganttProjects[0].principalInvestigatorName, daysRemaining: 90 }],
          trainingEventsUpcoming: [],
        } } });
      return;
    }
...
for (const width of widths) {
  test(`Progress Overview controls and Gantt remain responsive at ${width}px`, async ({ page }, testInfo) => {
    ...
    expect(firstKpi!.height).toBeLessThan(110);
    const kpiColumnCount = await page.locator(".overview-kpi-grid").evaluate((element) => getComputedStyle(element).gridTemplateColumns.split(" ").length);
    expect(kpiColumnCount).toBe(width === 1920 ? 6 : width === 375 ? 2 : 3);

    const timeSections = filterCard.locator(":scope > div");
    const timeSectionTops = await timeSections.evaluateAll((elements) => elements.slice(0, 3).map((element) => Math.round(element.getBoundingClientRect().top)));
    if (width >= 1280) expect(new Set(timeSectionTops).size).toBe(1);

    const filterControls = page.locator(".overview-filter-grid > :not(input)");
    const filterTops = await filterControls.evaluateAll((elements) => elements.map((element) => element.getBoundingClientRect().top));
    if (width >= 1280) expect(Math.max(...filterTops) - Math.min(...filterTops)).toBeLessThan(3);
    ...
    if (width === 1366) {
      ...
      await expect(page.getByText("Năm 2025", { exact: true })).toBeVisible();
      await page.getByRole("button", { name: "2026", exact: true }).click();
      await expect(page.getByText("Năm 2026", { exact: true })).toBeVisible();
    }
    ...
    const customRange = page.locator(".overview-custom-range");
    await expect(customRange).toBeVisible();
    expect((await customRange.boundingBox())!.y).toBeGreaterThan((await filterCard.boundingBox())!.y);
    ...
    await expect(page.getByTestId("gantt-header")).toHaveCSS("position", "static");
    if (width >= 1024) {
      const sidebarBox = await page.locator("aside").boundingBox();
      const ganttBox = await gantt.boundingBox();
      expect(ganttBox!.x).toBeGreaterThanOrEqual(sidebarBox!.x + sidebarBox!.width - 1);
    }
    await expect(page.getByRole("button", { name: "Chi tiết" }).first()).toBeVisible();
    const screenshotStates: Record<number, "expanded" | "collapsed"> = { 1024: "expanded", 1366: "expanded", 1440: "collapsed", 1920: "expanded" };
    const desiredState = screenshotStates[width];
    if (desiredState) {
      const toggle = page.getByRole("button", { name: /thanh bên/ });
      const currentLabel = await toggle.getAttribute("aria-label");
      const currentlyCollapsed = currentLabel?.startsWith("Mở rộng") ?? false;
      if ((desiredState === "collapsed") !== currentlyCollapsed) await toggle.click();
      await expect(toggle).toHaveAttribute("aria-label", desiredState === "collapsed" ? /^Mở rộng/ : /^Thu gọn/);
      await page.waitForTimeout(250);
      await expectNoPageOverflow(page);
    }
    await page.getByRole("button", { name: /^Cả năm/ }).click();
    await page.getByRole("button", { name: "2026", exact: true }).click();
    await expect(page.getByText(longProjectName, { exact: false }).first()).toBeVisible();
    await gantt.evaluate((element) => { element.scrollLeft = 0; });
    if (width === 1366) {
      const lastRow = page.getByTestId("gantt-row").last();
      await lastRow.scrollIntoViewIfNeeded();
      expect((await lastRow.boundingBox())!.y).toBeLessThan(1000);
      await page.evaluate(() => window.scrollTo(0, 0));
    }
    if (width === 375 || desiredState) await page.screenshot({ path: testInfo.outputPath(`progress-overview-${width}-${desiredState ?? "mobile"}.png`), fullPage: true });
  });
}

test("Progress Overview empty alerts stay compact", async ({ page }) => {
  await page.setViewportSize({ width: 1366, height: 900 });
  await authenticate(page, { emptyAlerts: true });
  await page.goto("/");
  const panels = page.locator(".overview-alert-grid > *");
  await expect(panels).toHaveCount(3);
  for (let index = 0; index < 3; index++) expect((await panels.nth(index).boundingBox())!.height).toBeLessThan(120);
  await expectNoPageOverflow(page);
});

for (const zoom of [1.25, 1.5]) {
  test(`Progress Overview remains compact at ${Math.round(zoom * 100)}% zoom-equivalent width`, async ({ page }) => {
    await page.setViewportSize({ width: Math.round(1440 / zoom), height: Math.round(900 / zoom) });
    await authenticate(page);
    await page.goto("/");
    await expectNoPageOverflow(page);
    const columns = await page.locator(".overview-kpi-grid").evaluate((element) => getComputedStyle(element).gridTemplateColumns.split(" ").length);
    expect(columns).toBeGreaterThanOrEqual(3);
    expect((await page.locator(".overview-kpi-grid > *").first().boundingBox())!.height).toBeLessThan(110);
  });
}
