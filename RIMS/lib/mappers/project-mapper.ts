...
export function mapApiProjectToUi(project: ApiResearchProject): ResearchProject {
  return {
    ...
    researchTypeCode: project.researchType ?? null,
    researchType: project.researchTypeName ?? project.researchType ?? "Khác",
    protocolNumber: project.protocolNumber ?? "",
    protocolVersion: project.protocolVersion ?? "",
    ethicsStatusCode: project.ethicsStatus,
    ethicsStatus: (project.ethicsStatusName ?? mapEthicsStatus(project.ethicsStatus, project.ethicsExpiryDate)) as ResearchProject["ethicsStatus"],
    ...
    status: (project.projectStatusName ?? mapProjectStatus(project.projectStatus)) as ResearchProject["status"],
    riskLevelCode: project.riskLevel,
    health: (project.riskLevelName ?? mapHealthStatus(project.riskLevel)) as ResearchProject["health"],
    currentPhaseCode: project.currentPhaseName ?? null,
    currentPhase: project.currentPhaseDisplayName ?? project.currentPhaseName ?? "Chưa bắt đầu",
...
export function mapGanttProjectToUi(project: DashboardGanttProjectDto): ResearchProject {
  return {
    ...
    sponsor: project.sponsorName ?? "Chưa có",
    sponsorCode: null,
    researchTypeCode: null,
    researchType: "",
    protocolNumber: "",
    protocolVersion: "",
    ethicsStatus: "Không yêu cầu",
    ethicsStatusCode: null,
    ethicsExpiry: null,
    ...
    riskLevelCode: project.healthStatus,
    currentPhaseCode: project.phases.find((p) => p.phaseStatus === "in_progress")?.phaseName ?? project.phases[0]?.phaseName ?? null,
